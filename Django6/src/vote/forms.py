...
#form : HTML 코드에서 사용할 입력양식(<input>)을 모델클래스에 맞게 (또는 커스텀) 자동으로 만들어주는 기능
...
class QuestionForm(ModelForm):
    class Meta: #Meta 클래스 정의를 통해서 모델 클래스에 관한 정보를 입력
        ...
        #fields : 모델클래스의 어떤 변수를 입력양식으로 만들것인지 지정하는 변수(interable)
        #exclude : 모델클래스의 어떤 변수를 입력양식에서 제외할 것인지 지정하는 변수(interable)
        #fields, exclude 벼수중 한개만 사용해야함
