...
def vote(request):
    if request.method == "POST": #사용자 요청이 post방식인 경우에 투표 처리를 함
        ...
        #결과 뷰의 URL을 반환
        return HttpResponseRedirect( reverse('vote:result', args=(c.question.id,)) )
    else: #잘못된 클라이언트 접근
...
from _datetime import datetime #현재시간/날짜를 불러오는 함수

#질문 등록 뷰
@login_required
def questionregister(request):
    if request.method == 'GET': #클라이언트가 처음 해당 뷰를 호출했을 경우
        #폼클래스의 객체 생성, 인자값을 전달하지 않은 경우 입력양식에 값이 비어있는 형태의 객체를 생성
        form1 = QuestionForm()
        print( form1.as_p() )
        
        return render(request, 'vote/qr.html', { 'a':form1 })
    elif request.method == 'POST': #클라이언트가 해당 뷰로 데이터를 보낸 경우
        #reuest.POST : POST방식으로 요청한 클러이언트의 데이터가 들어있는 변수
        form1 = QuestionForm( request.POST )
        if form1.is_valid(): #사용자가 입력한 값이 유효한 값인지 확인
            #폼객체.cleaned_data : is_valid() 함수로 유효한 값인지 확인한 후 True가 반환됬을 떄, 사용자의 입력정보를 확인할 수 있는 사전형 변수
            print('등록하려는 설문지 제목', form1.cleaned_data['name'])
            #폼객체.save() : 사용자가 입력한 데이터를 기반으로 연결된 모델클래스의 객체를 데이터베이스에 생성
            #form1.save() -> 사용자가 입력한 데이터로 Question 객체 생성 및 데이터베이스에 저장
            #form1.save(commit = False) -> 사용자가 입력한 데이터로 Question 객체 생성 및 변환
            
            #Question 객체는 date 변수에 값이 비어있으면 안되므로, 파이썬 코드로 변수값을 채워준 뒤에 저장해야함
            q = form1.save(commit = False) #QuestionForm 객체로 Question 객체를 생성(데이터베이스저장X)
            print('생성된Question 객체', q)
            q.date = datetime.now() #파이썬 모듈을 이용해서 현재시간을 Question객체의 date변수에 저장
            q.save()                #생성된 새로운 Question 객체를 데이터베이스에 저장
            
            return HttpResponseRedirect( reverse('vote:index') )
        else: #사용자가 입력한 갓이 유효한 갓이 아닌경우에 대한 처리
            #사용자가 입력한 데이터를 바탕으로 입력양식을 제공하는 HTML 문서 전달
            return render(request, 'vote/qr/html', {'a' : form1, 'error' : '유효하지않은 값을 작성했습니다.'})
...
def questionUpdate(request, question_id):
    ...
    if request.method == "GET":
        ...
        if form1.is_valid(): #수정한 값이 유한 값인지 확인
...
#POST -> 사용자 입력 데이터를 기반으로 ChocieForm 객체 생성 - > 유효한 값인지 확인 -> 객체 변환 및 데이터베이스에 저장 -> index 또는 detail URL을 반환
...
def choiceUpdate(request, c_id):
    ...
    if request.method == 'GET':
        ...
        if cf.is_valid():
            cf.save()
            return HttpResponseRedirect( reverse( 'vote:detali', args=(c.question.id,) ) )
        else:
            return render(request, 'vote/cr.html', {'a' : cf, 'error' : '유효한 값이 아닙니다.'})
        
#Choice 객체 찾기
#GET/POST 분리
#GET -> Choice 객체를 기반으로 ChoiceForm 생성 -> HTML전달
#POST -> 사용자입력데이터  + Choice 객체를 기반으로 ChoiceForm 생성 -> 유효한 값인지 확인 -> 객체저장 -> URL 반환
...
#4)index 또는 detail의 URL을 반환
    return HttpResponseRedirect( reverse('vote:detai', args=(c.question.id,) ) )
