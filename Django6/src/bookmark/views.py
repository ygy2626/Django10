...
#View : 사용자의 요청에 따라 처리하고, HTML 문서나 새로운 URL 주소를 전송하는 역할

#View 정의 시 클래스/함수 형태로 정의

#View 함수 정의 시 첫번째 매개변수로 request를 반드시 사용함
#request : 사용자의 요청 저오, <from>으로 넘겨준 데이터, 로그인정보, 세션정보, 요정방식(GET, POST)
...
def index(request):
    ...
    return render(request, 'bookmark/index.html', { 'a' : 'Hello Django', 'b' : [1,2,3] } )


#해당 모델클래스에 저장된 객체들을 추출하기 위해서 임포트
from .models import BookMark

#북마크 모데를래스에 저장된 객체를 HTML 문서에 전달하는 기능
def booklist(request):
    #모델클래스명.objects.all() : 데이터베이스에 해당 모델클래스로 저장된 모든 객체를 리스트 형태 추출
    #모델클래스명.objects.get() : 해당 모델클래스에 객체 중 특정 조건을 만족하는 객체 1개를 추출
    #         .objects.filter() : 특정 조건으 ㄹ만족하는 다수의 객체 리스트형태로 추출
    #         .objects.exclude() : 특정 조건을 만족하지않는 다수의 객체를 리스트형태로 추출
...
def bookdetail(request, book_id ):
    ...
    #객체들을 식별하기 위한 id 변수가 자동으로 생성되 특정객체를 추출할 때 사용할 수 있음
    #조건을 만족하는 객체가 하개도 없는 경우, 서버에러가 발생함
