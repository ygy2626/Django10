...
#생성한 파일로 migrate 를 통해 뎅터 베이스에 저장공간 생성 됨
#클래스 정의 -> makemigrations 를 통해 migration 파일을 생성
...
class BookMark(models.Model):
    ...
    #print(북마크객체) -> 해당 객체의 bookmark 변수에 저장된 문자열 출력
    def __str__(self):
        return self.bookname
    # 모델클래스를 정렬, 보여지는 이름을 변경할 때 Meta 클래스를 정의해 사요알 수 있음
    class Meta:
        ordering = ('bookname',) #정렬방식을 bookname변수의 갓을 오름찬순으로 정렬
        #내림차순으로 정렬하고 싶은 경우 변수명 앞에 '-' 붙이기