...
class Post(models.Model):
    ...
    #models.CASEADE : 연결된 객체가 삭제되면 다같이 삭제됨
    ...
    #null : 데이터베이스에 저장할 떄 해당 변수 값이 비어있어도 생성되도록 허용
    #blank : 폼객체.is_valid(), 폼객체.as_p 을 사용할 떄 빈칸을 허용
    content = models.TextField('내용', null=True, blank=True)
    #settings.AUTH_USER_MODEL : 현재 웹서버가 회원을 관리할 떄 사용하는 모델클래스를 의미
    author = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE)
    #DateTimeField의 매개변수 auto_now_add : 객체가 생성될때 자동으로 서버의 혀재시간을 저장할 수 있도록 허용
